...
//cout <<"Current_time in update window"<< current_time.get_msec() <<endl;
  cout << setw(4)<<noskipws<<fixed<<setfill(' ')<<setprecision(1)<< (double)(current_time.get_msec())/1000.0, 4,0 ;
  cout << "WorldModel|";
  cout << fps<<"fps|";
  //cout <<"vor MWMCurrent_time in update window"<< current_time.get_msec() <<endl;
  //double volt=MWM.get_robot_data(robotdata_time).motor_vcc;
//  cout <<"nach MWMCurrent_time in update window"<< current_time.get_msec() <<endl;
...
/*  if (wait_for_manual_start)
  ...
  //cout << '|' <<CONSOLE_RED<< setw(4)<<setfill(' ') <<((bl.pos-rl.pos)/rl.heading).x << "," << setw(4)<<setfill(' ') <<((bl.pos-rl.pos)/rl.heading).y << "," << setw(4)<<setfill(' ') <<bl.pos.z<< ")";
