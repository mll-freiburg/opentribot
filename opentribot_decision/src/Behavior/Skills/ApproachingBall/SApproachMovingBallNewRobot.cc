...
using namespace std;
  ...
  SApproachMovingBall::SApproachMovingBall(double maxVel) throw ()
    ...
    throw(TribotsException)
  {
    ...
    double kp=CONFIG_GETVAR("tune.cfg","Heading_kp",20,"HeadingController_p",0,200)*1.0f/10;
    double ki=CONFIG_GETVAR("tune.cfg","Heading_ki",0,"HeadingController_i",0,200)*1.0f/10;
    double kd=CONFIG_GETVAR("tune.cfg","Heading_kd",10,"HeadingController_d",0,200)*1.0f/10;
    double minturn=CONFIG_GETVAR("tune.cfg","Heading_minspeed",-60,"HeadingController_min",-100,100)*1.0f/10;
    double maxturn=CONFIG_GETVAR("tune.cfg","Heading_maxspeed",60,"HeadingController_max",-100,100)*1.0f/10;
   ...
   Vec ballvel = 
      MWM.get_ball_location(t).velocity.toVec();
...
//    cout<< "Sapproach Moving ball Relball.x "<< relBall.x <<" relball.y " << relBall.y<<endl;
    ...
    double PLF_V1x=CONFIG_GETVAR("tune.cfg","PLF_V1x",10,"HeadingController",0,200)*1.0f/100;
    double PLF_V1y=CONFIG_GETVAR("tune.cfg","PLF_V1y",40,"HeadingController",0,200)*1.0f/100;
    double PLF_V2x=CONFIG_GETVAR("tune.cfg","PLF_V2x",20,"HeadingController",0,200)*1.0f/100;
    double PLF_V2y=CONFIG_GETVAR("tune.cfg","PLF_V2y",50,"HeadingController",0,200)*1.0f/100;
    double PLF_V3x=CONFIG_GETVAR("tune.cfg","PLF_V3x",100,"HeadingController",0,200)*1.0f/100;
    double PLF_V3y=CONFIG_GETVAR("tune.cfg","PLF_V3y",100,"HeadingController",0,200)*1.0f/100;
    max_rotate_speed=CONFIG_GETVAR("tune.cfg","PLF_MAXROTATESPEED",35,"HeadingController",0,200)*1.0f/10;
...
//dv.vrot = -(fabs(hdiff)/hdiff)*max_rotate_speed*plf.getValue(fabs(hdiff));
    ...
    /*
    //ballangle: 
    Angle relballtofront=relBall.angle()-Angle::quarter;
    double relballtofrontdeg=relballtofront.get_deg();
    if  (fabs(relballtofrontdeg)>30 && fabs(relballtofrontdeg)<180)
        {

                if (   (   fabs (relBall.y) >250)   && (fabs(relBall.y) <500)       )
                {
                        LOUT<<"\n Noch nicht gut auf den ball ausgerichtet und nahe dran, nicht so schnell fahren";
                        dv.vtrans=dv.vtrans *0.6;
                        if (fabs(relballtofrontdeg)>90){
                        LOUT<<"\n Noch weniger gut ausgerichtet, noch langsamer fahren";
                        dv.vtrans=dv.vtrans *0.5;
                        }
...
//    if (  (fabs(robotPos.x+robotVel.x*1000) )> 2000+MWM.get_field_geometry().field_width/2){
//    dv.vtrans=dv.vtrans* 0.3;
//    LOUT<<"Fahre Nicht die  Seitenbande um";
//}
//    if (  (fabs(robotPos.y+robotVel.y*1000) )> 2000+MWM.get_field_geometry().field_length/2){
//    dv.vtrans=dv.vtrans* 0.3;
//    LOUT<<"Fahre Nicht die  Seitenbande um";
//}

    
//    dv.vtrans=dv.vtrans*0;		
...
int inhibit_translation=CONFIG_GETVAR("tune.cfg","inhibit_movement",0,"Verhindere fahren",0,1);
    if (inhibit_translation)
    {
      dv.vtrans = Vec(0,0);
    }
#endif    
