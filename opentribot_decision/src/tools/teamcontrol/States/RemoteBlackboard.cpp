...
TribotsTools::RemoteRobotState* TribotsTools::BlackboardState::get_robot_with_id(unsigned int id) {
  for (unsigned int i=0; i < robot_state.size(); i++) {
    if (robot_state[i].id == id) {
      return &(robot_state[i]);
    }
  }
  return 0;
}

const TribotsTools::RemoteRobotState* TribotsTools::BlackboardState::get_robot_with_id(unsigned int id) const {
  for (unsigned int i=0; i < robot_state.size(); i++) {
    if (robot_state[i].id == id) {
      return &(robot_state[i]);
    }
