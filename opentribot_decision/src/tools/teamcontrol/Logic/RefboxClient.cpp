...
//      cerr << "rcvd=" << rcvd[0] << '|' << rcvd[1] << '|' << rcvd[2] << '|' << rcvd[3] << '|' << rcvd[4] << '|' << rcvd[5] << '|' << rcvd[6] << '|' << rcvd[7] << '|' << rcvd[8] << '|' << rcvd[9] << "  (" << result << ")\n";
    if (result <0) {
    ...
    std::string pf = welcomePrefix+string(rcvd, result);
    bool is_prefix = (pf==string("Welcome..").substr(0,pf.length())) || (pf==string("Reconnect").substr(0,pf.length()));
    bool is_identical = (pf==string("Welcome..")) || (pf==string("Reconnect"));
    if (waitforwelcome && is_identical) {
      // die Welcome-Message vollstaendig gelesen
      logstream << now << " got welcome message = '" << pf << "'" << endl;
      welcomePrefix="";
      waitforwelcome=false;
      return res;
    } else if (waitforwelcome && is_prefix) {
      // direkt nach connect/reconnect den Anfang einer Welcome-Message gelesen
      logstream << now << " received message that might be the begining of a welcome message: '" << pf << "'" << endl;
      welcomePrefix=pf;
      return res;
    } else if (waitforwelcome) {
      // direkt nach connect/reconnect, aber die Zeichen ergaenzen sich nicht zu einer Welcome-Message
      logstream << now << " received incomplete welcome message: '" << welcomePrefix << "'" << endl;
      welcomePrefix="";
      waitforwelcome=false;
    }
    for (unsigned int i=0; i<10; i++)
      if (rcvd[i]!=0) {
        logstream << now << " received message from refbox: '" << rcvd[i] << "' = " << static_cast<int>(rcvd[i]) << "." << endl;
