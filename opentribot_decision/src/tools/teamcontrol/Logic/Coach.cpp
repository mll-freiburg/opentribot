...
void Coach::sl_mirror_hint () {
  ...
  if (num_max>num_snd_max && num_snd_max>0) {
    // es gibt eine Mehrheitsentscheidung und mindestens einen Roboter, der den Ball falsch sieht
    Vec pos (0,0);
    for (unsigned int i=0; i<num; i++) {
    ...
    for (unsigned int sender=0; sender<num_robots; sender++) {
      if (REMBB.robot_state[sender].in_game && !REMBB.robot_state[sender].comm_interrupted) {
        if (REMBB.robot_state[sender].ball_pos.pos_known==BallLocation::known) {
          if (clusters[i]!=index_max && pos.length()>1500
           && REMBB.robot_state[sender].ball_pos.pos.toVec().length()>1500
           && (REMBB.robot_state[sender].ball_pos.pos.toVec()+pos).length()<1500
           && (REMBB.robot_state[sender].ball_pos.velocity.toVec().length()<1.0)) {
            cerr << "Roboter " << REMBB.robot_state[sender].id << " moeglicherweise auf falscher Seite\n";
            LOUT << "Roboter " << REMBB.robot_state[sender].id << " moeglicherweise auf falscher Seite\n";
