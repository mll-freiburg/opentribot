...
public:
  ...
  TribotsTools::Policy* get_policy (const std::string&, const Tribots::ConfigReader& reader) throw (Tribots::TribotsException,std::bad_alloc) {
    return new TribotsTools::ChallengePlayerPolicy(reader);
  }
};
...
ChallengePlayerPolicy::ChallengePlayerPolicy(const ConfigReader& reader) 
  : StaticPolicy(3, reader), dynamicChangeCycleCounter(0)
{
