...
TeamcontrolFieldWidget::TeamcontrolFieldWidget(QWidget* p, Qt::WindowFlags f) : QMainWindow (p,f)
{
  setupUi(this);
  ...
  field->init (this, REMBB.team_state.field_geometry, ppref, psel);
...
TeamcontrolFieldWidget::~TeamcontrolFieldWidget ()
{
...
void TeamcontrolFieldWidget::paintEvent ( QPaintEvent * )
{
  PRINT("PAINT");
  unsigned int ref = field->get_preferences().reference_robot;
...
void TeamcontrolFieldWidget::showEvent ( QShowEvent * )
{
  QWidget::show();
  field->zoom_all();
}
...
{
  switch (event->key()) {
  case Qt::Key_G: //go tribots
    REMBB.team_state.refbox_signal = Tribots::SIGstart;
    break;
  case Qt::Key_Space: //stop playing
