...
namespace {
  ...
  enum {
    mouseModeShiftWindow=1, // Bewegung
    mouseModeZoomInWindow=2, // Zoom
    mouseModeShowVector=3, // Punkt und Richtung
    mouseModeSLHint=4, // SL zuruecksetzen auf Position
    mouseModeSLErrorWindow=5, // SL-Fenster o. ae.
    mouseModeAuxLines=6, // Hilfslinie setzen
    mouseModeSelectRobot=7, // Referenzroboter aendern
    mouseModeGotoPos=8 // Roboter anweisen, an Position zu fahren
  };
...
const unsigned int num_ftales = 27;
const std::string ftales [num_ftales] = {
  "Es gibt keine leichten Gegner",
  "Ein Spiel dauert 30 Minuten",
  "Nach dem Spiel ist vor dem Spiel",
  "Sechs Roboter muesst ihr sein",
  "Die Null muss stehen",
  "Der naechste Gegner ist immer der schwerste",
  "Entscheidend ist auf dem Platz",
  "Angriff ist die beste Verteidigung",
  "Der Ball ist rund",
  "Wer keine Tore schiesst, kann nicht gewinnen",
  "Die Verteidigung beginnt im Sturm",
  "Abseits ist, wenn der Schiedsrichter pfeift",
  "Mit Spruechen schiesst man keine Tore",
  "Die Schweden sind keine Hollaender",
  "Die Offensive gewinnt Spiele, die Defensive die Meisterschaft",
  "Sechs gute Roboter ergeben noch lange keine gute Mannschaft",
  "Entscheidend ist nicht die Aufstellung sondern die Einstellung",
  "Fuer schoen spielen gibt es keine Punkte",
  "Das Glueck muss man erzwingen",
  "Das Spiel wird im Prozessor entschieden",
  "Den Ball flach halten",
  "Der Pfosten ist der beste Freund des Torwarts",
  "Der RoboCup hat seine eigenen Gesetze",
  "Jedes Spiel beginnt bei null",
  "Man kann nur so gut spielen wie es der Gegner zulaesst",
  "Wenn es spielerisch nicht klappt, muss es ueber den Kampf gehen",
  "Wer vorne die Dinger nicht rein macht, darf sich nicht wundern wenn er am Ende verliert"
};

Tribots::Vec pftalespos;
std::string pftale;
Tribots::RingBuffer<std::string> recent_ftales (15);
  ...
  public:
    ...
    void drawRobot (Tribots::Vec p, Angle a, bool kick) {
      ...
      for (unsigned int i=0; i<6; i++)
        parr.setPoint (i, static_cast<int>(vertices[i].x), static_cast<int>(vertices[i].y));
      drawPolygon (parr);  // Begrenzung
...
FieldOfPlay::FieldOfPlay ( QWidget* parent, const char* name, WFlags fl) : QWidget (parent, name, fl), dock_area (NULL), toolbar_zoom (NULL), toolbar_show(NULL) {
  ...
  forceMouseSLHint=false;
  forceMouseGoto=false;
}
...
void FieldOfPlay::init (QMainWindow* mw, const Tribots::FieldGeometry& fg, const PaintPreferences& pp, const PaintActionSelect& paselect, Qt::Dock dock) {
  ...
  if (paselect.next_imagesource) {
    ...
    next_imagesource_act ->addTo (toolbar_show);
    next_imagesource_act ->setText ("Bildquelle umschalten");
    im.loadFromData (image10_data, sizeof( image10_data ), "PNG" );
    pm = im;
    next_imagesource_act -> setIconSet (pm);
    next_imagesource_act -> setToggleAction (false);
    connect (next_imagesource_act, SIGNAL(activated()), this, SLOT(next_imagesource()));
  }
...
void FieldOfPlay::forceMouseLocalizationClick (bool b) throw () {
  forceMouseSLHint=b;
}

void FieldOfPlay::forceMouseGotoClick (bool b) throw () {
  forceMouseGoto=b;
}

void FieldOfPlay::say () {
  bool is_in=false;
  do {
    is_in=false;
    std::string ft = ftales[static_cast<unsigned int>(num_ftales*urandom())];
    for (unsigned int i=0; i<recent_ftales.size(); i++) {
      if (ft==recent_ftales[i])
        is_in=true;
    }
    if (!is_in) {
      pftale = ft;
  ...
  pftalespos.x=width();
  pftalespos.y=dock_height+(height()-36-dock_height)*urandom();
}
...
void FieldOfPlay::paintEvent(QPaintEvent *) {
  ...
  QWMatrix mapping_f (0.0, sf*s, sf*s, 0.0, 0.5*width()-s*ppref.zoom.image_center.y, 0.5*height()-s*ppref.zoom.image_center.x+0.5*dock_height);
  QWMatrix mapping_i (0.0, s, s, 0.0, 0.5*width()-s*ppref.zoom.image_center.y, 0.5*height()-s*ppref.zoom.image_center.x+0.5*dock_height);
  if (ppref.zoom.orientation<0) {
     mapping_f = QWMatrix (sf*s,0.0,0.0,-sf*s,0.5*width()-s*ppref.zoom.image_center.x,0.5*height()+s*ppref.zoom.image_center.y+0.5*dock_height);
    mapping_i = QWMatrix (s,0.0,0.0,-s,0.5*width()-s*ppref.zoom.image_center.x,0.5*height()+s*ppref.zoom.image_center.y+0.5*dock_height);
  }

  paint.setWorldMatrix (mapping_i);
  if (ppref.show_direction) {
    paint.setPen(QColor(9,181,0));
    paint.setBrush(QBrush (QColor(9,181,0), Qt::SolidPattern));
    int yl=static_cast<int>(fgeometry.field_length/2-fgeometry.penalty_area_length-1000);
    int xl=static_cast<int>(fgeometry.center_circle_radius);
    QPointArray qvec (3);
    qvec.setPoint (0,-2*xl,yl/2);
    qvec.setPoint (1,0,yl);
    qvec.setPoint (2,2*xl, yl/2);
    paint.drawConvexPolygon (qvec);
    paint.drawRect (-xl, -yl, 2*xl, yl+yl/2);
  }

  // FTales
  if (pftale.length()!=0) {
    MyQPainter paint2 (this);
    paint2.setPen (QColor(127,255,127));
    QFont fn = paint2.font ();
    fn.setPointSize(36);
    fn.setBold (true);
    paint2.setFont (fn);
    paint2.drawText (static_cast<int>(pftalespos.x), static_cast<int>(pftalespos.y), pftale.c_str());
    pftalespos.x-=25;
  }

  // Feldlinien
  paint.setPen (Qt::gray);
  paintObjects (paint, field_lines2);
  paintObjects (paint, field_arcs2);
  paint.setPen (Qt::white);
  paintObjects (paint, field_lines);
  paintObjects (paint, field_arcs);

  // Tore
  if (ppref.show_colored_goals)
  ...
  if (ppref.show_wm_robot) {
    for (unsigned int i=0; i<rloc.size(); i++) {
      ...
      if (robot_active && !rloc[i].valid) {
        paint.setPen (QPen(Qt::yellow, 2));
        paint.drawArc (rloc[i].pos.x-500, rloc[i].pos.y-500, 1000, 1000, 0, 5760);  // delokalisierten Roboter mit gelbem Ring versehen
      }
    }
  }
  // Roboter-ID
  if (ppref.show_robot_ids) {
    ...
    for (unsigned int i=0; i<robot_ids.size() && i<rloc.size(); i++) {
      if (i==ppref.reference_robot) {
        ...
        paint2.setPen (QColor (196,0,196));
      } else {
        fn.setItalic (false);
  ...
  if (ppref.show_wm_obstacles) {
    ...
    fn.setBold (true);
    paint2.setFont (fn);
    for (unsigned int i=0; i<cinfo.oloc.size(); i++) {
      paint.setPen (QPen (Qt::black, 2));
      Tribots::Vec midpoint = cinfo.oloc[i].pos;
      double width = cinfo.oloc[i].width;
      ...
      if (cinfo.oloc[i].player>=0) {
        Vec p = field2widget (midpoint);
        string number = "x";
        if (cinfo.oloc[i].player>=10) {
          number = "xx";
          number[0]=cinfo.oloc[i].player/10+'0';
          number[1]=cinfo.oloc[i].player%10+'0';
        } else {
          number[0]=cinfo.oloc[i].player+'0';
        }
        paint2.drawText (static_cast<int>(p.x), static_cast<int>(p.y), QString(number.c_str()));
      }
      if (cinfo.oloc[i].velocity.length()>0) {
        paint.setPen (QPen (Qt::black, 1));
        paint.drawLine (midpoint.x, midpoint.y, midpoint.x+500*cinfo.oloc[i].velocity.x, midpoint.y+500*cinfo.oloc[i].velocity.y);
      }
    }
  }
  ...
  if (ppref.imagesource_id<cinfo.vloc.size() && (ppref.show_vis_lines || ppref.show_vis_ball || ppref.show_vis_obstacles || ppref.show_vis_goals)) {
    ...
    for (unsigned int i=0; i<cinfo.vloc[ppref.imagesource_id].objectlist.size(); i++) {
      ...
      if (cinfo.vloc[ppref.imagesource_id].objectlist[i].object_type == VisibleObject::ball && ppref.show_vis_ball) {
        ...
        paint.setPen (QPen (Qt::darkRed, 2, Qt::DotLine));
        paint.drawArc (p.x-0.5*fgeometry.ball_diameter, p.y-0.5*fgeometry.ball_diameter, fgeometry.ball_diameter, fgeometry.ball_diameter, 0.0, 5760.0);
...
void  FieldOfPlay::mousePressEvent(QMouseEvent* ev) {
  ...
  if (forceMouseSLHint)
    mouse_mode=mouseModeSLHint;
  else if (forceMouseGoto)
    mouse_mode=mouseModeGotoPos;
  else if (ev->button() == Qt::LeftButton && ev->state() == Qt::NoButton )
    mouse_mode=mouseModeShiftWindow;
  else if (ev->button() == Qt::RightButton && ev->state() == Qt::NoButton )
    mouse_mode=mouseModeZoomInWindow;
  else if (ev->button() == Qt::LeftButton && ev->state() == Qt::ShiftButton )
    mouse_mode=mouseModeShowVector;
    ...
    mouse_mode=mouseModeSLErrorWindow;
    ...
    mouse_mode=mouseModeAuxLines;
  else if (ev->button() == Qt::RightButton && ev->state() == Qt::ControlButton )
    mouse_mode=mouseModeSelectRobot;
  else {
    mouse_mode=0;
    update();
  }
...
Tribots::Vec FieldOfPlay::widget2field (Tribots::Vec u) const {
  ...
  if (ppref.zoom.orientation>=0) {
    ...
    z.y=-(u.y-0.5*dock_height-0.5*height())/s+ppref.zoom.image_center.y;
  }
  return z;
}
...
Tribots::Vec FieldOfPlay::field2widget (Tribots::Vec z) const {
  ...
  if (ppref.zoom.orientation>=0) {
    ...
    u.y = 0.5*height()+0.5*dock_height-s*(z.y-ppref.zoom.image_center.y);
  }
  return u;
}
...
void FieldOfPlay::show_colored_goals (bool b) {
  ppref.show_colored_goals = b;
  emit (preferencesChanged());
  update();
}

void FieldOfPlay::show_direction (bool b) {
  ppref.show_direction = b;
  emit (preferencesChanged());
  update();
}
...
void FieldOfPlay::set_field_geometry (const Tribots::FieldGeometry& fg2) throw () {
  fgeometry=fg2;
  fgeometry.make_lineset (field_lines, field_arcs, true);
  fgeometry.make_lineset (field_lines2, field_arcs2, false);
}

void FieldOfPlay::paintObjects (QPainter& painter,  const std::vector<Tribots::LineSegment>& objs) {
  for (std::vector<Tribots::LineSegment>::const_iterator it = objs.begin(); it!=objs.end(); it++)
    painter.drawLine (rd(it->p1.x), rd(it->p1.y), rd(it->p2.x), rd(it->p2.y));
}

void FieldOfPlay::paintObjects (QPainter& painter,  const std::vector<Tribots::Arc>& objs) {
  for (std::vector<Tribots::Arc>::const_iterator it = objs.begin(); it!=objs.end(); it++)
    painter.drawArc (rd(it->center.x-it->radius), rd(it->center.y-it->radius), rd(2*it->radius), rd(2*it->radius), rd((it->start+Angle::quarter).get_deg()*(-16)), rd((it->end-it->start).get_deg()*16));
}
