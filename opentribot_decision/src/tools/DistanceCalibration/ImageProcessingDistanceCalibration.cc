...
void ImageProcessingDistanceCalibration::process_images() throw (Tribots::BadHardwareException) {
  ...
  if (MWM.get_game_state().in_game && MWM.get_game_state().refstate==Tribots::freePlay && ballRegions.list.size()>0 && fabs(MWM.get_recent_drive_vector().vrot)>0.2) { // Ball gefunden
