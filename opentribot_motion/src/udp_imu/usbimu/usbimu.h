...
#define PRODUCTID		0x233F
#define VENDORID2		0x03A3
#define PRODUCTID2		0x2B49
...
unsigned char buffer[CNTRL_BUF_SIZE];
...
if  ( 
	...
	((dev->descriptor.idVendor == VENDORID2)
...
ret =
      usb_control_msg (udev, bmRTmask, 1, 0, 0,(char*) buffer, CNTRL_BUF_SIZE,
		       TIMEOUT);
...
//    
acc[1] = (buffer[3] << 2) + ((buffer[5] >> 4) & 0x03);
//    
acc[2] = (buffer[4] << 2) + ((buffer[5] >> 6) & 0x03);
      acc[0] = (buffer[2] << 2) + ((buffer[5] >> 3) & 2);
    
acc[1] = (buffer[3] << 2) + ((buffer[5] >> 4) & 2);
    
acc[2] = (buffer[4] << 2) + ((buffer[5] >> 5) & 2);
...
joy[1] = buffer[0] * 1.0f / 128;
  //  printf ("\na1 %f  a2 %f  a3 %f", acc[0], acc[1], acc[2]);
      ...
      // y p r
      gyro[0] = (((buffer[6 + 3]>>2) << 8) + buffer[6 + 0]);
    
gyro[1] = (((buffer[6 + 5] >>2)<<8)  + buffer[6 + 2]);
    
gyro[2] = (((buffer[6 + 4] >>2) << 8) + buffer[6 + 1]);
    


slow[0] = (buffer[6 + 3] & 0x02) ;
    
slow[1] = (buffer[6 + 3] & 0x01) ;
    
slow[2] = (buffer[6 + 4] & 0x02) ;
...
ret =
      usb_control_msg (udev, bmRTmask, 2, 0, 0,(char*) buffer, CNTRL_BUF_SIZE,
		       TIMEOUT);
