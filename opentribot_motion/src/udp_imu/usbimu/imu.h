...
struct V3d
{
  ...
  V3d (double x_, double y_, double z_)
  {
    x = x_;
    y = y_;
    z = z_;
...
struct Quaternion
{
  ...
  double z;
    ...
    Quaternion (double _r, double _x, double _y, double _z)
  {
    r = _r;
    x = _x;
    y = _y;
  ...
  void set (double _r, double _x, double _y, double _z)
  {
    r = _r;
    x = _x;
    y = _y;
...
// // combine values to generate the vector and scalar for the quaternion
  ...
  double dot_product_with (Quaternion q)
  {
 ...
 */
  ...
  {
     if (measured > hi) measured=hi-1;
     if (measured < lo) measured=lo+1;
    float x = (hi - lo)*0.5f;
        float ret=(float) ((measured - mid) / x);
//        printf( " conv2g: %f",ret);
      return ret;
...
// raw values
  ...
  bool debug;
  enum Direction{ACCX,ACCY,ACCZ};
  enum RotationAxis{ROLL,YAW,PITCH};
  Axis accaxis[3];
  Axis gyroaxis[3];
  int accmap[3];
  int gyromap[3];
  ...
  enum calibratestate{FLAT,RIGHT,FRONT,TURN};
  
  int acc_calibrate_axis(calibratestate s,int axisnum,double value);
  int gyro_calibrate_axis(calibratestate s,int axisnum,double value);
  int ax, ay, az;
  int y, p, r, jy, jx;
  int bc;
  int bz;
...
V3d  axisrotate;
  ...
  int gyro_calibrate_zero ();
  int accelerometer_calibrate(calibratestate s);
  int savecalibration();
  int loadcalibration();
