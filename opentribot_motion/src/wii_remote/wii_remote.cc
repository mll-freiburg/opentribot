...
if (argc<4){
        cout << "usage: "<<argv[0]<< " TargetIP"<<"  max_speed";
        cout << " WiimoteAddr"<<endl;
...
cout<< "Prüfe gleich"<<endl;

if (strcmp(argv[3],"ANY")==0){
cout << "using ANY wiimote"<<endl;
bdaddr=*BDADDR_ANY;
}
else{ 
wiimote1=argv[3];
...
//bdaddr=*BDADDR_WIIMOTE1;
	...
	printf("Connecting to Motionserverr...");
...
printf("\n yeah %i",cyclecounter);
cyclecounter++;
if(cyclecounter%200==0)	rumble(wiimote,100);
...
if (state.buttons & CWIID_BTN_HOME){
	...
	cout << "EXITING ...... XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"<<endl;;	
	if (cwiid_close(wiimote)) {
		fprintf(stderr, "Error on wiimote disconnect\n");
		exit(-1);
	}
...
if (CWIID_MESG_STATUS==CWIID_MESG_ERROR)exit(-1);
