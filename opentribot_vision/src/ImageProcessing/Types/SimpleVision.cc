...
namespace {
  ...
  public:
    Builder () {
      Tribots::ImageProcessingFactory::get_image_processing_factory ()->sign_up (std::string("Simple"), this);
    }
    Tribots::ImageProcessingType* get_image_processing (const std::string&, const std::string& section, const Tribots::ConfigReader& reader, Tribots::ImageProcessingType*, const Tribots::ImageProducer* producer) throw (Tribots::TribotsException,std::bad_alloc) {
      return new Tribots::SimpleImageProcessing (reader, section.c_str(), producer->getRobotMask(), producer->getImageCenter(), producer->getImageWorldMapping(), producer->getWorldImageMapping(), producer->getWidth(), producer->getHeight());
    }
  };
  Builder the_builder;
}
...
namespace Tribots {
  ...
  SimpleImageProcessing::SimpleImageProcessing(const ConfigReader& config, 
                                                   const char* section,
      ...
      preferClosestBall(true)
  {
    ...
    ballDetector = 
      new BallLearner(robotMask, image2real, real2image);
  ...
  SimpleImageProcessing::~SimpleImageProcessing() throw()
  {  
  ...
  void 
  SimpleImageProcessing::process_image(Image* image, VisibleObjectList* vol, RegionList* ballRegions) 
...
//    LOUT<<"SCANLINE: NBALLS: " << nBalls << endl;
    ...
    else if (nBalls > 1) {  // TODO: 3D-Ballposition wo moeglich verwenden
...
//      double maxX = 0.5*fg.field_width+fg.side_band_width;
      double maxY = 15000;
      double maxX = 20000;
      //double maxYinField = 0.5*fg.field_length;
      //double maxXinField = 0.5*fg.field_width;
      Frame2d rel2abs(Vec(0,0),Angle(0));     // TODO: ggf. 3D-Position verwenden
      Vec firstWorld = rel2abs * image2real->map(potentialBallRegions.list[0]->getCenterOfGravity());
      if (preferClosestBall &&               // wenn baelle Nahe beim Roboter bevorzugt werden sollen
          ...
          fabs(firstWorld.y) < maxY) {       // dann suche den ball, der am n�chsten am roboter ist
                // implizit: potentialBallRegions enth�lt aktuell entweder nur b�lle auf dem Feld oder
                //           nur B�lle au�erhalb des Feldes.
        // gucken, ob irgendein ball im feld ist (ohne sidebands)
      /*  bool reallyinField=false;
        for (unsigned int i=0; i < potentialBallRegions.list.size(); i++)
        {
          Vec testWorld = rel2abs * image2real->map(potentialBallRegions.list[i]->getCenterOfGravity());
          if (fabs(testWorld.x) < maxXinField &&
              fabs(testWorld.y) < maxYinField)
            reallyinField = true;
        }*/
        double minSDist = image2real->map(potentialBallRegions.list[0]->getCenterOfGravity()).squared_length();
        int minPos = 0;
        double tmp;
    //    if (reallyinField) LOUT << "ReallyInField" << endl;
        for (unsigned int i=1; i < potentialBallRegions.list.size(); i++) {
/*          if (reallyinField)
          {
            // es gibt einen ball der im feld liegt, also minimalen abstand zu diesen bestimmen
            Vec testWorld = rel2abs * image2real->map(potentialBallRegions.list[i]->getCenterOfGravity());
            if (fabs(testWorld.x) < maxXinField && fabs(testWorld.y) < maxYinField && 
                (tmp = image2real->map(potentialBallRegions.list[i]->getCenterOfGravity()).squared_length()) <= minSDist) {
              minSDist = tmp;
              minPos = i;
            }
          } else
          {*/
            // es gibt nur baelle in der sideband
            if ((tmp = image2real->map(potentialBallRegions.list[i]->getCenterOfGravity()).squared_length()) <= minSDist) {
              minSDist = tmp;
              minPos = i;
            }            
//          }
...
//    LOUT << "SCANLINE: BallRegions zu GMSV: " << ballRegions->list.size() << endl;
  ...
  void SimpleImageProcessing::request_image_raw() throw(TribotsException)
  {
    if (imageCollected) {
  ...
  void SimpleImageProcessing::request_image_processed() throw(TribotsException)
  {
    if (imageCollected) {
      throw TribotsException("You have collected an image, but have not "
			     "freed the image yet.");
  ...
  bool SimpleImageProcessing::is_image_available() throw()
  {
    return requestedImage != 0;
  }
  
  const Image* SimpleImageProcessing::get_image() 
  ...
  void SimpleImageProcessing::free_image() throw(TribotsException)
  {
    if (! imageCollected) {
  ...
  void SimpleImageProcessing::visualizeCommonInfo() 
  {
    RGBTuple rgb;
