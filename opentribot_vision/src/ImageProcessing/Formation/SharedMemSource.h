...
namespace Tribots {
  ...
  class SharedMemSource : public ImageSource
  {
  public:
    /** reads relevant parameters from config-reader Arg1 using section Arg2 */
    SharedMemSource(const ConfigReader&, const std::string&) throw (HardwareException, InvalidConfigurationException);
    /** reads images from File Arg1 */
    SharedMemSource(const std::string&) throw (HardwareException);
    virtual ~SharedMemSource() throw ();
