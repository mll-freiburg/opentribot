...
double Tribots::Line::distance (const Tribots::Vec p) const throw () {
  Vec d=p2-p1;
...
void Tribots::Line::draw(std::ostream& stream) const throw () {
...
void Tribots::LineSegment::draw(std::ostream& stream) const throw () {
  stream << " line " << p1 << p2;
}
...
Vec Tribots::Circle::closest_point (const Vec p) const throw () {
...
void Tribots::Circle::draw(std::ostream& out) const throw () {
...
Tribots::Vec Tribots::intercept_theorem(const Tribots::Vec& point, double b, double h)
{
...
Tribots::Vec Tribots::Line::perpendicular_point (const Tribots::Vec p) const throw () {
  Vec d=p2-p1;
  return p1+((p*d-p1*d)/(d.squared_length()))*d;
}
Tribots::Vec Tribots::Line::closest_point (const Tribots::Vec p) const throw () {
...
Tribots::Vec Tribots::LineSegment::closest_point (const Tribots::Vec p) const throw () {
...
Tribots::Vec Arc::closest_point (const Vec p) const throw () {
  Vec v = p-center;
  if (v.angle().in_between (start, end))
    return center+radius*v.normalize();
  else {
    Tribots::Vec p1 = center+radius*Vec::unit_vector (start);
    Tribots::Vec p2 = center+radius*Vec::unit_vector (end);    
...
Arc& Arc::s_rotate   (const Angle& a) throw() {
  center.s_rotate (a);
  start+=a;
  end+=a;
  return (*this);
}
Arc& Arc::s_translate(const Vec& v) throw() {
  center+=v;
  return (*this);
}
Arc Arc::rotate (const Angle a) const throw () {
  Arc ret (*this);
  ret.s_rotate(a);
  return ret;
}
  
Arc Arc::translate (const Vec v) const throw () {
  Arc ret (*this);
  ret.s_translate (v);
  return ret;
}
...
void Tribots::Arc::draw (std::ostream& stream) const throw () {
  stream << " arc " << center << radius << ' ' << start.get_deg() << ' ' << end.get_deg();
}
...
std::ostream& operator<< (std::ostream& stream, const Tribots::Curve& curve) {
  curve.draw (stream);
  return stream;
}

std::ostream& operator<< (std::ostream& stream, const Tribots::Area& area) {
  area.draw (stream);
  return stream;
}

std::ostream& operator<< (std::ostream& stream, const Tribots::CurveAndArea& curve_area) {
  const Area& area (curve_area);
  area.draw (stream);
  return stream;
}

std::vector<Vec> Tribots::intersect (const Circle& cc, const Quadrangle& qd) throw (std::bad_alloc) {
  std::vector<Vec> res;
  std::vector<Vec> im;
  LineSegment ls [4];
...
std::vector<Vec> Tribots::intersect (const LineSegment& l, const Circle& c) throw (std::bad_alloc) {
  Line ln (l.p1, l.p2);
...
std::vector<Vec> Tribots::intersect (const Line& al, const Quadrangle& qd) throw (std::bad_alloc) {
  std::vector<Vec> res;
  std::vector<Vec> im;
  LineSegment ls [4];
  ls[0]=LineSegment (qd.p1, qd.p2);
...
std::vector<Vec> Tribots::intersect (const LineSegment& al, const Quadrangle& qd) throw (std::bad_alloc) {
  std::vector<Vec> res;
  std::vector<Vec> im;
  LineSegment ls [4];
  ls[0]=LineSegment (qd.p1, qd.p2);
